...
public class HttpUtils {
     ...
     * @return byte array of data
     */
    public static String getStringData(String ip, int webPort) throws Exception {
        if (!client.isStarted()) {
            client.start();
        }

        ContentResponse response = client.GET("http://" + ip + ":" + webPort + "/jsonrpc.js");
        return response.getContentAsString();
//        String urlString = "http://" + ip + ":" + webPort + "/jsonrpc.js";
//        URL url = new URL(urlString);
//        URLConnection connection = url.openConnection();
